...
// managed-integrity-v1: 2HVW3DWpDX9
//
...
mavenPublishing {
    coordinates("one.wabbit", "kotlin-ij-plugin-common", project.version.toString())
    publishToMavenCentral()
    signAllPublications()
    pom {
        name.set("kotlin-ij-plugin-common")
        description.set("Shared IntelliJ-side helper library for Wabbit Kotlin compiler-plugin IDE support modules.")
        url.set("https://github.com/wabbit-corp/kotlin-ij-plugin-common")
        licenses {
            license {
        ...
        scm {
            url.set("https://github.com/wabbit-corp/kotlin-ij-plugin-common")
            connection.set("scm:git:git://github.com/wabbit-corp/kotlin-ij-plugin-common.git")
            developerConnection.set("scm:git:ssh://git@github.com/wabbit-corp/kotlin-ij-plugin-common.git")
        }
    }
}
...
val intellijSdkArchive by configurations.creating
val extractedIntellijSdkDir = layout.buildDirectory.dir("intellij-sdk/ideaIC-2025.3")
val extractIntellijSdk by tasks.registering(Sync::class) {
    from({ zipTree(intellijSdkArchive.singleFile) })
    into(extractedIntellijSdkDir)
    includeEmptyDirs = false
}
val intellijSdkClasspath =
    files(
        extractedIntellijSdkDir.map { it.dir("lib") },
        extractedIntellijSdkDir.map { it.dir("plugins/Kotlin/lib") }
    ).builtBy(extractIntellijSdk).asFileTree.matching {
        include("**/*.jar")
    }
...
dependencies {
    ...
    intellijSdkArchive("com.jetbrains.intellij.idea:ideaIC:2025.3@zip")
    compileOnly(intellijSdkClasspath)
    testImplementation(intellijSdkClasspath)
}
...
kover {
    ...
    //             // title.set("kotlin-ij-plugin-common Code Coverage") // Uncomment to customize title
    //         }
    //         xml {
...
plugins.withId("org.jetbrains.dokka") {
    afterEvaluate {
        extensions.findByType(DokkaExtension::class)?.apply {
            moduleName.set("kotlin-ij-plugin-common")
            ...
            dokkaSourceSets.named("main").configure {
                ...
                sourceLink {
                    ...
                    remoteUrl("https://github.com/wabbit-corp/kotlin-ij-plugin-common/tree/master/src/main/kotlin")
